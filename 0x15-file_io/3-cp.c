 ...
 * @fd1: second descriptor to close
 ...
 * 98 if file_form does not exist or unreadable
 * 99 if write fails
 * 100 if file close fails else 0
 * 0 otherwise
 */
int main(int ac, char *av[])
{
  ...
  if (ac != 3)
    {
      dprintf(STDERR_FILENO, "Usage: cp file_file_from file file_to\n");
      return (97);
    }
  file_from = open(av[1], O-RDONLY);
  if (file_from == -1)
    {
      dprintf(STDERR_FILENO, "Erro: can't read from file %s\n", av[1]);
      return (98);
    }
  file_to = open(av[2], O_WRONLY | 0_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
  if (file_to == -1)
    {
      dprintf(STDERR_FILENO, "Error: can't write to %s\n", av[2]);
  ...
  do {
    ...
    if (lenr == -1)
      return (read-err(file_from, file_to, av[1]));
